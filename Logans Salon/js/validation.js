let phoneRegex = /^(\+\d{1,3}[- ]?)?\d{10}$/; //TODO go to regexer.com or reg101.com and find a suitable phone number regex and place here
...
function validateState(id, msg) {
  ...
  Upel = el.value.toUpperCase();
  //check whether the value is in the stateAbbreviations array
  if (stateAbbreviations.includes(Upel)) {
    valid = true;
  }
...
function checkRequired(id, message) {
  ...
  switch (type) {
    ...
    case "checkbox":
    case "radio":
      let checks = document.querySelectorAll("[name = find-page]");
      let check;
      // console.log(check);
      for (check of checks) {
        if (!check.checked) {
